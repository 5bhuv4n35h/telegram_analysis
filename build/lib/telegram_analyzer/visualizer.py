...
class Visualizer:
    ...
    def generate_all_visualizations(self):
        ...
        try:
            ...
            if self._try_generate_visualization(self.generate_chat_velocity_chart):
                successful_viz.append('chat_velocity.png')

            if self._try_generate_visualization(self.generate_engagement_intelligence_chart):
                successful_viz.append('engagement_intelligence.png')

            if self._try_generate_visualization(self.generate_psychological_wellbeing_chart):
                successful_viz.append('psychological_wellbeing.png')

            if self._try_generate_visualization(self.generate_mood_distribution_chart):
                successful_viz.append('mood_intensity.png')
    ...
    def generate_weekly_activity_chart(self):
        ...
        plt.figure(figsize=(12, 6))
        ax = sns.barplot(x=list(day_counts.index), y=list(day_counts.values), hue=list(day_counts.index), palette='viridis', legend=False)
    ...
    def generate_emoji_usage_chart(self):
        ...
        ax = sns.barplot(x='count', y='emoji', data=emoji_df, hue='emoji', palette='viridis', legend=False)
    ...
    def generate_word_frequency_chart(self):
        ...
        try:
            ...
            plt.figure(figsize=(12, 8))
            import seaborn as sns
            ax = sns.barplot(x='count', y='word', data=word_df, hue='word', palette='viridis', legend=False)
    ...
    def generate_chat_velocity_chart(self):
        """Generate message velocity chart over time"""
        self.logger.info("Generating chat velocity chart")
        daily_velocity = self.df.groupby(self.df['date'].dt.date).size()
        rolling_7 = daily_velocity.rolling(window=7, min_periods=1).mean()
        
        plt.figure(figsize=(14, 7))
        plt.plot(daily_velocity.index, daily_velocity.values, alpha=0.3, label='Daily Volume', color='gray')
        plt.plot(rolling_7.index, rolling_7.values, label='7-Day Momentum', color='#0088cc', linewidth=3)
        
        plt.fill_between(rolling_7.index, rolling_7.values, alpha=0.1, color='#0088cc')
        
        plt.title('Chat Momentum & Message Velocity')
        plt.xlabel('Timeline')
        plt.ylabel('Messages / Day')
        plt.legend()
        plt.grid(True, alpha=0.1)
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'chat_velocity.png'))
        plt.close()

    def generate_engagement_intelligence_chart(self):
        """Generate engagement scoring visualization for users"""
        self.logger.info("Generating engagement intelligence chart")
        
        from telegram_analyzer.analyzer import ChatAnalyzer
        analyzer = ChatAnalyzer(self.df)
        intel = analyzer.get_engagement_intelligence()
        
        if not intel:
            return
            
        users = list(intel.keys())
        scores = [d['engagement_score'] for d in intel.values()]
        
        # Sort by score
        sorted_indices = np.argsort(scores)[-10:] # Top 10
        top_users = [users[i] for i in sorted_indices]
        top_scores = [scores[i] for i in sorted_indices]
        
        plt.figure(figsize=(12, 8))
        colors = plt.cm.viridis(np.linspace(0, 1, len(top_users)))
        bars = plt.barh(top_users, top_scores, color=colors)
        
        plt.title('User Intelligence & Engagement Rankings')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'engagement_intelligence.png'))
        plt.close()

    def generate_psychological_wellbeing_chart(self):
        """Generate analysis of health and care mentions over time"""
        self.logger.info("Generating psychological wellbeing chart")
        
        # Resample to daily counts
        daily_health = self.df.groupby(self.df['date'].dt.date)['health_mentions'].sum()
        daily_care = self.df.groupby(self.df['date'].dt.date)['care_mentions'].sum()
        
        plt.figure(figsize=(14, 7))
        plt.plot(daily_health.index, daily_health.values, label='Health & Wellness', color='#ff4d4d', linewidth=2, alpha=0.8)
        plt.plot(daily_care.index, daily_care.values, label='Love & Care', color='#ff3399', linewidth=2, alpha=0.8)
        
        plt.fill_between(daily_health.index, daily_health.values, alpha=0.15, color='#ff4d4d')
        plt.fill_between(daily_care.index, daily_care.values, alpha=0.15, color='#ff3399')
        
        plt.title('Psychological Wellbeing & Care Timeline')
        plt.xlabel('Timeline')
        plt.ylabel('Mentions')
        plt.legend()
        plt.grid(True, alpha=0.1)
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'psychological_wellbeing.png'))
        plt.close()

    def generate_mood_distribution_chart(self):
        """Generate mood intensity heatmap"""
        self.logger.info("Generating mood distribution chart")
        
        # Average intensity by hour and day
        mood_pivot = pd.crosstab(
            self.df['date'].dt.day_name(),
            self.df['date'].dt.hour,
            values=self.df['mood_intensity'],
            aggfunc='mean'
        ).fillna(0)
        
        days_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
        mood_pivot = mood_pivot.reindex(days_order)
        
        plt.figure(figsize=(12, 8))
        sns.heatmap(mood_pivot, cmap='RdPu', annot=False, cbar_kws={'label': 'Emotional Intensity'})
        plt.title('Emotional Intensity Heatmap (Mood Pulse)')
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'mood_intensity.png'))
        plt.close()

    def generate_user_interaction_network(self):
        """Generate network visualization of user interactions"""
        self.logger.info("Generating user interaction network")
        
        try:
            import networkx as nx
            
            # Create interaction edges (simplified: sequence of messages)
            edges = []
            users = self.df['from'].dropna().tolist()
            for i in range(len(users) - 1):
                if users[i] != users[i+1]:
                    edges.append(tuple(sorted((users[i], users[i+1]))))
            
            edge_counts = Counter(edges)
            
            G = nx.Graph()
            for (u1, u2), weight in edge_counts.items():
                if weight > 1: # Only significant interactions
                    G.add_edge(u1, u2, weight=weight)
            
            if len(G.nodes) == 0:
                self.logger.warning("Not enough interactions for network graph")
                return False
            ...
            nx.draw_networkx_nodes(G, pos, node_size=node_sizes, node_color='#0088cc', alpha=0.7)
            
            # Draw edges
            weights = [G[u][v]['weight'] for u, v in G.edges()]
            max_w = max(weights) if weights else 1
            nx.draw_networkx_edges(G, pos, width=[w/max_w * 5 for w in weights], alpha=0.3, edge_color='gray')
            
            # Draw labels
            nx.draw_networkx_labels(G, pos, font_size=10, font_family='sans-serif', font_color='white')
            
            plt.title('Participant Interaction Network', color='white', size=15)
            plt.axis('off')
            plt.tight_layout()
            
            # Dark theme adjustment for labels if needed
            plt.gcf().set_facecolor('#0e1621')
            
            output_path = os.path.join(self.output_dir, 'user_interaction_network.png')
            plt.savefig(output_path, facecolor='#0e1621')
            plt.close()
            
            self.logger.info(f"Interaction network saved to {output_path}")
            return True
        except Exception as e:
            self.logger.error(f"Error generating interaction network: {str(e)}")
            return False
