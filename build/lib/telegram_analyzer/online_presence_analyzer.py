...
class OnlinePresenceAnalyzer:
    ...
    def get_online_time_stats(self) -> Dict:
        ...
        # New: Response Latency Analysis
        latency_stats = self._analyze_response_latency()
    ...
    """
        ...
        if 'user_activity_heatmap.png' in viz_files:
            html += """
                                        ...
                                        <p class="text-muted">High-level view of activity by day and hour.</p>
            """
            
        # Add collective heatmap
        if 'presence_heatmap.png' in viz_files:
            html += """
                                        <h4>Collective Presence Heatmap (24/7)</h4>
                                        <img src="presence_heatmap.png" class="viz-img" alt="Presence Heatmap">
                                        <p class="text-muted">A deep dive into the group's "awake" hours across the entire week.</p>
            """
        ...
        if 'response_time_distribution.png' in viz_files:
            html += f"""
                                        ...
                                        <div class="p-3 bg-light rounded mt-2">
                                            <h6>Median Response Latency</h6>
                                            <p class="small text-muted">Fastest: {", ".join([f"{u} ({l:.1f}m)" for u, l in stats.get('fastest_responders', [])])}</p>
                                        </div>
            """
    ...
    def generate_online_presence_visualizations(self) -> List[str]:
        ...
        try:
            ...
            if ghosting_scores:
                ...
                ax = sns.barplot(x='Ghosting Score', y='User', data=ghosting_df, hue='User', palette='viridis', legend=False)
            ...
            # 5.5. Presence Heatmap
            self._generate_presence_heatmap()
            generated_viz.append('presence_heatmap.png')
            ...
            if consistency_scores:
                ...
                ax = sns.barplot(x='Consistency Score', y='User', data=consistency_df.head(10), hue='User', palette='viridis', legend=False)
    ...
    def _analyze_response_latency(self) -> Dict:
        """Analyze how quickly users respond on average"""
        latencies = {}
        for user in self.messages_df["from"].unique():
            # Only consider responses within a 2-hour window to avoid skewing by session gaps
            user_responses = self.messages_df[
                (self.messages_df["from"] == user) & 
                (self.messages_df["is_response"]) & 
                (self.messages_df["time_since_prev"] <= 120)
            ]
            if len(user_responses) >= 5:
                latencies[user] = float(user_responses["time_since_prev"].median())
        sorted_latencies = sorted(latencies.items(), key=lambda x: x[1])
        return {
            "response_latencies": latencies,
            "fastest_responders": sorted_latencies[:3],
            "slowest_responders": sorted_latencies[-3:]
        }

    def _generate_presence_heatmap(self):
        """Generate a collective 24/7 activity heatmap"""
        try:
            heatmap_data = self.messages_df.groupby(["hour", "day_of_week"]).size().unstack(fill_value=0)
            days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
            heatmap_data = heatmap_data.reindex(columns=[d for d in days if d in heatmap_data.columns])
            plt.figure(figsize=(14, 10))
            sns.heatmap(heatmap_data, cmap="YlGnBu", annot=True, fmt="d")
            plt.title("Collective Presence Heatmap (Hour vs Day)")
            plt.xlabel("Day of Week")
            plt.ylabel("Hour of Day (24h)")
            plt.tight_layout()
            plt.savefig(os.path.join(self.output_dir, "presence_heatmap.png"))
            plt.close()
        except Exception as e:
            self.logger.error(f"Error generating presence heatmap: {str(e)}")
