...
class AnalyzerWebServer:
    ...
    def __init__(self, output_dir, host='127.0.0.1', port=5000, open_browser=True, 
        ...
        self.analysis_status = {}  # Thread-safe storage for analysis progress
    ...
    def _create_app(self):
        ...
        def upload_file():
            ...
            if request.method == 'POST':
                ...
                if file:
                    ...
                    self.logger.info(f"File upload started: {filename}")
                    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                    file.save(file_path)
                    self.logger.info(f"File saved to: {file_path}")
                    
                    # Start analysis in a background thread
                    self.analysis_status[username] = {
                        'running': True,
                        'progress': 0,
                        'status': 'running',
                        'message': 'Initializing neural engines...'
                    }
        ...
        def status():
            ...
            if not valid or username not in self.analysis_status:
                # If analysis not in tracker but report exists, it's done
            ...
            status_data = self.analysis_status[username]
        ...
        def request_entity_too_large(e):
            self.logger.error("Upload failed: File too large")
            flash('File too large (max 500MB)', 'error')
            return redirect('/upload')
    ...
    def _run_analysis(self, file_path, username=None):
        ...
        try:
            ...
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 10
                self.analysis_status[username]['message'] = "Parsing chat structure..."
            ...
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 30
                self.analysis_status[username]['message'] = "Analyzing communication patterns..."
            ...
            self.logger.info("Analyzing online presence patterns...")
            ...
            try:
                ...
                online_analyzer.generate_online_presence_report()
                online_presence_stats = online_analyzer.get_online_time_stats()
                analysis_results['online_presence'] = online_presence_stats
            except Exception as e:
                self.logger.error(f"Error in online presence analysis: {str(e)}")
            
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 60
                self.analysis_status[username]['message'] = "Applying psychological & emotional filters..."
            ...
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 70
                self.analysis_status[username]['message'] = "Generating interactive deep-dives..."
            ...
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 80
                self.analysis_status[username]['message'] = "Finalizing interactive visualizations..."
            ...
            # Update progress
            if username:
                self.analysis_status[username]['progress'] = 100
                self.analysis_status[username]['running'] = False
                self.analysis_status[username]['status'] = 'complete'
            
        except Exception as e:
            self.logger.error(f"Error during analysis: {str(e)}", exc_info=True)
            if username:
                self.analysis_status[username]['running'] = False
                self.analysis_status[username]['status'] = 'failed'
